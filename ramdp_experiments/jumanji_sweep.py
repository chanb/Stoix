...
# cnn+ variants), see stoix/networks/torso_compute.py, plus
# TransformerChainOfThoughtTorso (TRANSFORMER_ARCHES), see
# stoix/networks/torso_compute_transformer.py. *Not*
# TransformerExplicitCoTTorso (EXPLICIT_COT_ARCHES), which has no such param
...
# HALTING_TEMPERATURE_ARCHES, does *not* include mlp/gru/iru (those torsos'
# halting heads are still a bare nn.Dense(1) - the MLP option hasn't been
# added there) or EXPLICIT_COT_ARCHES (TransformerExplicitCoTTorso has no
# separate halting head at all - halting is a discrete token choice, see
# stoix/networks/torso_compute_explicit_cot.py). Used to pick whether
...
class Job:
    ...
    def group_tag_parts(self) -> List[str]:
        ...
        step budget, gamma, network hparams, PPO hparams, misc flags - instead
    ...
    def command(self, python_bin: str) -> List[str]:
        ...
        if self.arch in HALTING_TEMPERATURE_ARCHES:
            ...
            # (plain `=`, not `++`); no such param on EXPLICIT_COT_ARCHES yet.
...
def build_grid(args: argparse.Namespace) -> List[Job]:
    ...
    for env in args.envs:
        ...
        for (
            ...
            # system (same applicability as halting_ent_coef/use_dpo_loss
            # above) - forced to a non-expectile default for
