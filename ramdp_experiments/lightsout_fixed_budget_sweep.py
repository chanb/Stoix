...
grid, stacked as two channels (see the env's module docstring) - flattened
to a `2 * grid_size`-length vector via `stoa.FlattenObservationWrapper`.
...
Grid axes:
  ...
  - use_latent_feedback: latent feedback decoding
                 ...
                 stoix/networks/torso_compute_explicit_cot.py. transformer_explicit_cot
                 only (every other architecture has no such param, so this is forced to
                 a single value for them). Default False.
  - use_sandwich_norm: sandwich LayerNorm placement
                 ...
                 TransformerBlock docstring. transformer/transformer_explicit_cot only
                 (TRANSFORMER_ARCHES + EXPLICIT_COT_ARCH - every other architecture has no
                 TransformerBlock, so this is forced to a single value for them). Default
                 False.
...
Usage:
  ...
  python ramdp_experiments/lightsout_fixed_budget_sweep.py \\
...
# un-capped tag past 128 (see the "5x5_weight_decay_search" commands in
# experiments.md, e.g. one real config here already reaches 124/128 with
# only --latent-kl-coef added on top).
...
class Job:
    ...
    def group_tag_parts(self) -> List[str]:
        ...
        prefixes (b/g/hd/lr/clr/ec/nl/nh/md/ep/mb/clip/deta/ln/iln/radv/cba/sn/rms/dpo/iql)
        ...
        WandBLogger) gets unwieldy at run_name's length otherwise. Capped at
        ...
        parts = [self.env, f"{system_short}-{arch_short}", f"b{self.budget}", f"g{self.gamma:g}"]
...
def build_grid(args: argparse.Namespace) -> List[Job]:
    ...
    # PPO_SYSTEMS (ff_ppo.py/ff_ppo_explicit_cot.py) - forced to the first
    # requested value for every other system in the main product loop below,
    # then deduplicated by run_name, mirroring how delightful_combos/
    # num_heads_options collapse axes that don't apply.
    ppo_combos = list(
        itertools.product(
            args.epochs,
    ...
    #  - use_layer_norm only exists on AdaptiveComputationTimeTorso (mlp).
    #  - use_input_layer_norm exists on mlp/transformer/gru/iru/
    #    transformer_explicit_cot (transformer_explicit_cot has no
    #    use_layer_norm though, same as transformer/gru/iru - see
    #    NO_LAYER_NORM_ARCHES).
    ...
    for (
        ...
        # (unlike latent_kl_coef above, both ff_ppo.py's own systems and
        # ff_ppo_explicit_*) - forced to a non-DPO default for
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        "transformer_explicit_cot, transformer_explicit_cot_merged}. iru_unshared "
    ...
    parser.add_argument(
        ...
        "architecture in {mlp}; ignored (forced off) otherwise, since those torsos have "
        "no such param.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        "its original behavior. Same applicability as --num-heads/--mlp-dim; ignored (forced to "
        "the first value) for every other architecture.",
    )
    parser.add_argument(
        "--vocab-size",
        default="32",
    ...
    parser.add_argument(
        ...
        "to architecture in {transformer, transformer_explicit_cot} (every other architecture "
        "has no TransformerBlock); ignored (forced to the first value) otherwise. Default false.",
    )
    parser.add_argument(
        "--use-rmsnorm",
        default="false",
    ...
    for d in args.qkv_dim:
        assert d >= 0, f"qkv_dim must be >= 0 (0 means unset), got {d}"
    ...
    print(
        f"  use_layer_norm={args.use_layer_norm} (mlp only) "
    ...
    print(
        ...
        "(transformer/transformer_explicit_cot only)"
    )
    print(
