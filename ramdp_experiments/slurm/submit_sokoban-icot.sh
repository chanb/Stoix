...
for seed in "${seeds[@]}"; do
    ...
    # CMD="python ramdp_experiments/jumanji_sweep.py --systems ff_ppo_reinforce --max-steps 5 --base-seed ${seed} ${COMMON} --halting-ent-coef 0.01 --halting-temperature 1.0 --clip-halting-head false"
    # CMD_B64="$(base64 -w0 <<< "${CMD}")"
    # sbatch --export=ALL,CMD_B64="${CMD_B64}" --job-name="sokoban-icot-maxsteps5-seed${seed}" "${RUN_SCRIPT}"
done
