...
for seed in "${seeds[@]}"; do
    ...
    sbatch --export=ALL,CMD="${CMD}" --job-name="sokoban-icot-budget123-seed${seed}" "${RUN_SCRIPT}"

    CMD="python ramdp_experiments/jumanji_fixed_budget_sweep.py --systems ff_ppo_reinforce --budget 4 --base-seed ${seed} ${COMMON}"
    sbatch --export=ALL,CMD="${CMD}" --job-name="sokoban-icot-budget4-seed${seed}" "${RUN_SCRIPT}"

    CMD="python ramdp_experiments/jumanji_fixed_budget_sweep.py --systems ff_ppo_reinforce --budget 5 --base-seed ${seed} ${COMMON}"
    sbatch --export=ALL,CMD="${CMD}" --job-name="sokoban-icot-budget5-seed${seed}" "${RUN_SCRIPT}"

    CMD="python ramdp_experiments/jumanji_sweep.py --systems ff_ppo_reinforce --max-steps 5 --base-seed ${seed} ${COMMON} --halting-ent-coef 0.1 --halting-temperature 5.0"
    sbatch --export=ALL,CMD="${CMD}" --job-name="sokoban-icot-maxsteps5-seed${seed}" "${RUN_SCRIPT}"
done
