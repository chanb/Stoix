...
Companion to sokoban_sweep.py: instead of letting the actor's compute torso
...
`budget` at a fixed, non-adaptive cost, comparable to sokoban_sweep.py's
...
gamma is fixed at 0.9999, matching sokoban_sweep.py. total_timesteps defaults
...
Usage:
  python ramdp_experiments/sokoban_fixed_budget_sweep.py --dry-run                # preview the grid
  python ramdp_experiments/sokoban_fixed_budget_sweep.py --limit 6 --dry-run       # preview a slice
  python ramdp_experiments/sokoban_fixed_budget_sweep.py                           # run the full sweep
  python ramdp_experiments/sokoban_fixed_budget_sweep.py --systems ff_reinforce --architectures mlp \\
...
def main() -> None:
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=REPO_ROOT / "results_sokoban_fixed_budget_sweep",
