#!/usr/bin/env python
"""Hyperparameter sweep for RAMDP systems on Sokoban.
...
Grid axes:
  ...
  - c_max:       max pondering / CoT steps (network.actor_network.pre_torso.max_steps)
...
gamma is fixed at 0.9999 and +env.reward.step_penalty=0.0, matching the Sokoban
commands in ramdp_experiments/ramdp.md. total_timesteps defaults to 2e7 (a
reduced sweep budget) rather than the 1e8 used for full runs -- re-run the
winning config(s) at full budget afterwards.
...
Usage:
  python ramdp_experiments/sokoban_sweep.py --dry-run                # preview the grid
  python ramdp_experiments/sokoban_sweep.py --limit 6 --dry-run       # preview a slice
  python ramdp_experiments/sokoban_sweep.py                           # run the full sweep
  python ramdp_experiments/sokoban_sweep.py --systems ff_reinforce --architectures mlp \\
      --c-max 1,8 --hidden-dim 16 --lr 3e-4 --seeds 1             # small pilot run
"""
...
ARCH_TO_NETWORK = {
    "ff_reinforce": {"mlp": "mlp_compute", "transformer": "transformer_compute"},
    "ff_qac_fac": {"mlp": "mlp_compute_qac", "transformer": "transformer_compute_qac"},
    "ff_qac_naive": {"mlp": "mlp_compute_qac", "transformer": "transformer_compute_qac"},
}
...
class Job:
    ...
    def run_name(self) -> str:
        return (
            f"{self.system}-{self.arch}-c_max_{self.c_max}"
    ...
    def command(self, python_bin: str) -> List[str]:
        ...
        cmd = [
            ...
            f"network.actor_network.pre_torso.max_steps={self.c_max}",
...
def build_grid(args: argparse.Namespace) -> List[Job]:
    jobs = []
    for system, arch, c_max, hidden_dim, lr, critic_lr, seed in itertools.product(
        args.systems, args.architectures, args.c_max, args.hidden_dim, args.lr, args.lr, range(args.seeds)
    ):
        jobs.append(
            Job(
                system=system,
...
def main() -> None:
    ...
    parser.add_argument(
        "--architectures", default="mlp,transformer", help="Comma-separated subset of {mlp, transformer}."
    )
    parser.add_argument("--c-max", default="1,2,4,8", help="Comma-separated max pondering/CoT steps.")
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=REPO_ROOT / "results_sokoban_sweep",
    ...
    args.c_max = [int(x) for x in args.c_max.split(",")]
    ...
    for a in args.architectures:
        assert a in ("mlp", "transformer"), f"unknown architecture {a!r}"
    ...
    print(f"  c_max={args.c_max} hidden_dim={args.hidden_dim} lr={args.lr} seeds=0..{args.seeds - 1}")
