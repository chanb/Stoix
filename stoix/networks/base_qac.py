...
`.q_value(obs, compute_time=None)` (a state-action-value head Q(s, ·) or
Q(s, ·, c)) as separate `apply(..., method=...)` entry points sharing the
same embedding, so V and Q are always trained from - and read off - the same
representation rather than two independently-initialised networks. See
`stoix.systems.ramdp_vpg.ff_ppo` for how the two heads are used together
...
class ValueAndQCritic(nn.Module):
    ...
    def _q_input(
        self, embedding: chex.Array, compute_time: Optional[chex.Array]
    ) -> chex.Array:
        """The input `q_head` reads: just the shared embedding, or - for the
        "cond_naive"/"cond_fac" `qac_variant`s (see `ff_ppo.py`) - the
        embedding concatenated with a single learned linear feature of
        `compute_time` (a genuine `nn.Dense` mapping `c -> R`, not a
        table/one-hot lookup), so `q_head` can learn a dependence on `c`
        directly instead of only ever seeing Q(s,·,1) ("fac") or needing a
        `(num_actions, max_steps)`-shaped output ("naive")."""
        if compute_time is None:
            return embedding
        c = compute_time.astype(jnp.float32)[..., jnp.newaxis]
        c_feature = self.compute_time_dense(c)
    ...
    def q_value(
        ...
        """
        return self.q_head(self._q_input(self._embed(observation), compute_time))

    def __call__(
        self, observation: Observation, compute_time: Optional[chex.Array] = None
    ) -> chex.Array:
        embedding = self._embed(observation)
        return self.value_head(embedding), self.q_head(self._q_input(embedding, compute_time))
