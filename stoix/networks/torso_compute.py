...
class UnsharedIRUAdaptiveComputationTimeTorso(nn.Module):
    """Like `IRUAdaptiveComputationTimeTorso`, but with no weight sharing
    across pondering steps: each of the `max_steps` steps gets its own
    independently-parameterized `IRUStep` (a fresh stack of `num_layers`
    `IRUCell`s plus a halting head), rather than one shared step reused at
    every iteration. So this is a genuinely deep, unshared stack of up to
    `max_steps` distinct IRU layers - not a weight-tied recurrent block -
    ...
    used", not "how many times was the shared step applied." The running
    cell state (`carry`) still flows forward from one step's IRU layer into
    the next's, exactly as in the shared version - only the parameters at
    each depth are independent.

    Deliberately a separate class from `IRUAdaptiveComputationTimeTorso` (not
    a shared-vs-unshared flag on it), since the two have a genuinely
    different parameter count/growth behaviour as `max_steps` changes -
    increasing `max_steps` here adds a whole new IRU layer's worth of
    parameters, whereas for the shared version it's free.

    Same sampled/replayed/deterministic halting modes, `min_steps`/
    `max_steps` semantics, `num_layers` stacking *within* each step (see
    `IRUStep`), a zero-initialized `C_0`, final `tanh`, and latent-
    convergence diagnostics as `IRUAdaptiveComputationTimeTorso` - only how
    `step_fn` is instantiated differs (see `__call__`).
    ...
    def __call__(
        ...
        if not replaying and not deterministic:
            if rng is None:
                raise ValueError(
                    "rng must be provided to UnsharedIRUAdaptiveComputationTimeTorso when "
                    "sampling (i.e. target_compute_time is None and deterministic=False)."
                )
        if not (1 <= self.min_steps <= self.max_steps):
            raise ValueError(
                f"min_steps must be between 1 and max_steps ({self.max_steps}), "
        ...
        # Encoded once up front, then re-fed into each step's (separately-
        # parameterized) recurrent block - unlike `AdaptiveComputationTimeTorso`,
        # which only uses the observation to seed the very first state.
        input_embedding = nn.Dense(self.hidden_dim, kernel_init=self.kernel_init)(observation)
        if self.use_input_layer_norm:
            input_embedding = nn.LayerNorm()(input_embedding)
        ...
        # `carry` holds every stacked layer's own cell state
        # (`(num_layers, *batch_shape, hidden_dim)`), threaded between
        # pondering steps. `state` (below) is just the top layer's carry -
        # what everything else here (diagnostics, halting, the final
        # returned embedding) actually uses, same as when `num_layers == 1`.
        carry = jnp.zeros((self.num_layers,) + batch_shape + (self.hidden_dim,))
        state = carry[-1]
        ...
        for step in range(self.max_steps):
            prev_state = state
            # A fresh IRUStep instance per step (unique `name`), unlike
            # IRUAdaptiveComputationTimeTorso's single `step_fn` created once
