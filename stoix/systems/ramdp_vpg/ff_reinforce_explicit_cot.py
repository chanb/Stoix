...
This is a variant of `stoix.systems.ramdp_vpg.ff_reinforce` whose actor's
...
def get_distribution_act_fn_with_compute_time(
    ...
    an actor whose torso also emits explicit thought tokens (see
    `stoix.networks.torso_compute_explicit_cot.TransformerExplicitCoTTorso`),
    i.e. returns `(action_distribution, compute_time, thought_tokens)`. The
    ...
    def act_fn(
        ...
        if config.arch.evaluation_greedy:
            ...
            # token at each step, to match the deterministic (mode) action.
...
def get_learner_fn(
    ...
    def _update_step(
        ...
        # Fold compute cost into the discounting (Semi-MDP/RAMDP view) - see
        # `ff_reinforce.py` for the full derivation of why scaling both the
        # reward and the discount by `gamma ** compute_time` gives
        ...
        def _actor_loss_fn(
            ...
            # RERUN NETWORK. Replay (rather than re-sample) the halting-and-token
            # trajectory that was actually taken during rollout (`compute_times`
            # and `thought_tokens`, from the stored transition), mirroring how
            # `log_prob(actions)` below evaluates the stored action rather than
            ...
            # action they led to are all trained jointly, from the same
